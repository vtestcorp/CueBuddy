...
//	public AppiumDriver<MobileElement> driver;
	...
	public ReminderPage(AppiumDriver driver, ExtentTest test) {
		this.driver = driver;
		...
		j=new JavascriptClick(driver);
	}

	// Occasions
	@FindBy(xpath = "//android.view.View[@checked='true']/android.widget.RadioButton")
...
//	@FindBy(xpath = "//android.view.View[@content-desc='success Reminder Created successfully. Ok']")
	...
	public void click_Date(String date, String month, String year) throws InterruptedException {
		...
		MobileElement seekBar = (MobileElement) driver.findElement(
				MobileBy.AndroidUIAutomator("new UiSelector().className(\"android.widget.SeekBar\").index(1)"));
		MobileElement seekBarM = (MobileElement) driver.findElement(
				MobileBy.AndroidUIAutomator("new UiSelector().className(\"android.widget.SeekBar\").index(2)"));
		MobileElement seekBarY = (MobileElement) driver.findElement(
				MobileBy.AndroidUIAutomator("new UiSelector().className(\"android.widget.SeekBar\").index(3)"));
		Thread.sleep(3000);
		...
		while (!seekBarM.getAttribute("content-desc").equals(month)) {
			...
			System.out.println("End point - " + end);
			act.press(PointOption.point(y + 100, start + 20)).moveTo(PointOption.point(y + 20, (end / 3) + start - 3))
					.release().perform();
		}
		// For Year
	...
	public void select_Relation(String input_Relation) throws IOException {
		WebElement relation = driver
				.findElement(By.xpath("//android.view.View[@content-desc='" + input_Relation + "']"));
	...
	public void dragndrop() throws Exception {
		...
		act.longPress(PointOption.point(middleX, middleY)).moveTo(PointOption.point(leftX, middleY)).release()
				.perform();
	...
	public void click_DeleteSuccessOk_Button() throws Exception {
		// applyWait.waitForElementToBeClickable(deleteAlertButton, 30).click();
		Thread.sleep(5000);
		act.tap(PointOption.point(350, 715)).perform();
	...
	public void getcoordinate(String name) throws InterruptedException {
		...
		WebElement s = driver.findElement(By.xpath(
				"/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.FrameLayout/android.widget.FrameLayout/android.view.View/android.view.View/android.view.View/android.view.View/android.view.View/android.view.View[2]/android.view.View/android.view.View"));
		...
		for (MobileElement mobileElement : e) {
			if (mobileElement.getAttribute("content-desc") != null) {
				...
				try {
					...
					Screenshots.takeScreenshot(driver, name + nameEdited + " is varified");
					test.log(Status.INFO, name + nameEdited + " is varified");
	...
	public void verifyOccasionName(String name) throws InterruptedException {
		Thread.sleep(9000);
		...
		for (MobileElement mobileElement : e) {
			if (mobileElement.getAttribute("content-desc") != null) {
				boolean b = mobileElement.getAttribute("content-desc").contains(name);
				Assert.assertTrue(mobileElement.getAttribute("content-desc").contains(name));
				try {
